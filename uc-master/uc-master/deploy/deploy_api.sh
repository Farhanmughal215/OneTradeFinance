...
pid=$(ps -ef | grep uc-0.0.1-SNAPSHOT.jar | grep -v grep | grep fjly | awk '{print $2}')
if [ -n "$pid" ];then
        echo "killing pid:$pid"
        kill -9 $pid
fi
nohup java -Xms3072M -Xmx3072M -Xss256k -XX:MetaspaceSize=256m -XX:MaxMetaspaceSize=512m -XX:MaxDirectMemorySize=1024m -XX:+UseG1GC -XX:+UseCompressedOops -XX:+UseCompressedClassPointers -XX:+PrintCommandLineFlags -jar -DENABLE_TASK=false ./uc-0.0.1-SNAPSHOT.jar  --spring.profiles.active=fjly >> /home/work/logs/uc/$nowdate.log 2>&1 &