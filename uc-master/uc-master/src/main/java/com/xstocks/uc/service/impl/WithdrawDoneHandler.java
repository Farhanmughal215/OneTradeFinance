...
public class WithdrawDoneHandler implements TodoHandler {
    ...
    public boolean careAbout(TodoPO todo) {
        return TodoStatusEnum.ACCEPT.getCode() == todo.getTodoStatus() &&
                TodoTypeEnum.WITHDRAW.getCode() == todo.getTodoType();
    }
    ...
    public void process(TodoPO todo) {
        ...
        if (Objects.nonNull(user)) {
            user.setAvailableAssets(
                    user.getAvailableAssets().subtract(NumberUtils.createBigDecimal(todo.getDescription())));
